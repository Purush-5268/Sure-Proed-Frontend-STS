...
export default function FeedbackWidgetModal({ handleClose, feedbackState, setFeedbackState, showClose }) {
  ...
  useEffect(() => {
    const fetchExistingFeedback = async () => {
      try {
        const res = await apiClient.get(`${API_ENDPOINTS.FEEDBACK.BASE}?feedback_type=${feedbackType}`);
  ...
  const submitFeedback = async () => {
    ...
    try {
      ...
      setTimeout(() => handleClose(true), 2000); // true indicates forced close
    } catch (err) {
