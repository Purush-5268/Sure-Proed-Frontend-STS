  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <div>
          <h1>{showSeeded ? "Seeded Students Dashboard" : "Student Management Dashboard"}</h1>
          <p>{showSeeded ? "Monitor admin-created accounts pending first login." : "Filter and review all students."}</p>
        </div>
        <div className={styles.actions}>
          ...
          <Link to="/trustee/volunteer/add-student" className={styles.addButton}>+ Add Student</Link>
        </div>
      </div>

      {/* Volunteer-specific Stats omitted to keep it compact and performant. Focus is on the list below. */}
                  ...
                  <tr key={student.id}>
                    <td>
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <span style={{ color: 'var(--vol-text-secondary)', fontSize: '13px', marginTop: '2px' }}>
                          {student.student_code || "No Code"}
                        </span>
                      </div>
                    </td>
                    <td>
                      <span className={styles.active} style={{ marginBottom: "4px" }}>
                        {courseName || "No Course"}
                      </span>
                      <span style={{ display: "block", fontSize: "12px", color: "var(--vol-text-secondary)", fontWeight: "600" }}>
                        {cohortCode
                          ? cohortCode
                    ...
                    <td>
                      <button
                        onClick={() => navigate(`/trustee/volunteer/student-details/${student.id}`)}
                        className={styles.actionBtn}
                      >
                        Inspect
                      </button>
      ...
      {/* Pagination Controls */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1.5rem" }}>
          ...
          <span style={{ fontSize: "14px", fontWeight: "600", color: "var(--vol-text-secondary)" }}>Page {page}</span>
          <button
