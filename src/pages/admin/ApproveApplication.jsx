...
function ApproveApplication() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiClient.patch(API_ENDPOINTS.APPLICATIONS.BY_ID(id), {
        status: "QUALIFIED",
        remarks: remarks.trim() || "Application approved by admin.",
      });
  ...
  if (loading) return <div className={styles.container}><div className="premium-card"><h1>Approve Application</h1><SkeletonLoader variant="form" rows={4} /></div></div>;
  if (error) return <div className={styles.container}><div className="premium-card"><h1>Approve Application</h1><p style={{ color: "#b91c1c" }}>{error}</p></div></div>;
  if (!application) return <div className={styles.container}><div className="premium-card"><h1>Approve Application</h1><p>No application found.</p></div></div>;
      ...
      <div className="premium-card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className={styles.buttons}>
            <button type="submit" disabled={loading}>{loading ? "Saving..." : "Approve Application"}</button>
          </div>
        </form>
      </div>
    </div>
